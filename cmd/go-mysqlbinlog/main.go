...
func main() {
	...
	// get current binlog pos if the given param is none

	c, err := client.Connect(fmt.Sprintf("%s:%d", *host, *port), *user, *password, "")
	if err != nil {
		fmt.Println("Create Connection error: ", err.Error())
	}
	rr, err := c.Execute("show master status")
	binFile, _ := rr.GetString(0, 0)
	binPos, _ := rr.GetInt(0, 1)
	...
	pos := mysql.Position{Name: binFile, Pos: uint32(binPos)}
	if len(*backupPath) > 0 {
